module WhalePoolsDex.PContracts.PDoubleRoyaltyRedeem (
    doubleRoyaltyRedeemValidatorT
) where
...
import WhalePoolsDex.PContracts.PDoubleRoyaltyPool (extractPoolConfig)

doubleRoyaltyRedeemValidatorT :: ClosedTerm (RedeemConfig :--> OrderRedeemer :--> PScriptContext :--> PBool)
doubleRoyaltyRedeemValidatorT = plam $ \conf' redeemer' ctx' -> unTermCont $ do
    ...
    pure $
        pmatch action $ \case
            Apply -> unTermCont $ do
                ...
                poolConf       <- pletFieldsC @'["treasuryX", "treasuryY", "firstRoyaltyX", "firstRoyaltyY", "secondRoyaltyX", "secondRoyaltyY"] poolInputDatum
                let
                    treasuryX = getField @"treasuryX" poolConf
                    treasuryY = getField @"treasuryY" poolConf

                    firstRoyaltyX = getField @"firstRoyaltyX" poolConf
                    firstRoyaltyY = getField @"firstRoyaltyY" poolConf

                    secondRoyaltyX = getField @"secondRoyaltyX" poolConf
                    secondRoyaltyY = getField @"secondRoyaltyY" poolConf
                ...
                let 
                    ...
                    minReturnX = calcMinReturn # liquidity # inLq # poolValue # x # (treasuryX + firstRoyaltyX + secondRoyaltyX)
                    minReturnY = calcMinReturn # liquidity # inLq # poolValue # y # (treasuryY + firstRoyaltyY + secondRoyaltyY)
