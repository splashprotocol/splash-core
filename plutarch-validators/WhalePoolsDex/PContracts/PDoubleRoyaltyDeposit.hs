module WhalePoolsDex.PContracts.PDoubleRoyaltyDeposit (
    doubleRoyaltyDepositValidatorT
) where
...
doubleRoyaltyDepositValidatorT :: ClosedTerm (DepositConfig :--> OrderRedeemer :--> PScriptContext :--> PBool)
doubleRoyaltyDepositValidatorT = plam $ \conf' redeemer' ctx' -> unTermCont $ do
    ...
    pure $
        pmatch action $ \case
            Apply -> unTermCont $ do
                ...
                reservesX <- tlet $ (assetClassValueOf # poolValue # x) - treasuryX - firstRoyaltyX - secondRoyaltyX
                reservesY <- tlet $ (assetClassValueOf # poolValue # y) - treasuryY - firstRoyaltyY - secondRoyaltyY
