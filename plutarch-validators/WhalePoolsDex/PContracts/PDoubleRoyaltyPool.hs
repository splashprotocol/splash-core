...
readPoolState = phoistAcyclic $
    plam $ \conf' out -> unTermCont $ do
        ...
        let
            ...
            poolXSecondRoyalty = getField @"firstRoyaltyX" conf
            poolYSecondRoyalty = getField @"firstRoyaltyY" conf
...
correctSwapConfig = plam $ \prevDatum newDatum dx dy -> unTermCont $ do
  ...
  let
    ...
    prevsecondRoyaltyPubKey = getField @"firstRoyaltyPubKey" prevConfig
    ...
    deltaSecondRoyalty = 
      pif
        (zero #< dx)
        (newSecondRoyaltyX - prevfirstRoyaltyX)
        (newSecondRoyaltyY - prevfirstRoyaltyY)
