module WhalePoolsDex.PContracts.PPoolDAOV1 (
  daoMultisigPolicyValidatorT
)  where
...
validateCommonFieldsAndSiganture :: PMemberFields PoolConfig '["poolNft", "poolX", "poolY", "poolLq", "lqBound", "treasuryX", "treasuryY", "nonce"] s as 
  => HRec as
  -> HRec as
  -> Term s 
...
daoMultisigPolicyValidatorT daoPhs threshold lpFeeIsEditable = plam $ \redeemer' ctx' -> unTermCont $ do
  ...
  prevConf <- pletFieldsC @'["poolNft", "poolX", "poolY", "poolLq", "feeNum", "treasuryFee", "treasuryX", "treasuryY", "DAOPolicy", "lqBound", "treasuryAddress", "nonce"] poolInputDatum
  newConf  <- pletFieldsC @'["poolNft", "poolX", "poolY", "poolLq", "feeNum", "treasuryFee", "treasuryX", "treasuryY", "DAOPolicy", "lqBound", "treasuryAddress", "nonce"] poolOutputDatum
  let  
    ...
    -- Checks that main pool properties: tokenX, tokenY, tokenLq, tokenNft, feeNum aren't modified and sigantures are correct
  ...
  pure $ correctTxInputsQty #&& feeUtxoContainsOnlyAda #&& validCommonFieldsAndSigantures #&& validAction