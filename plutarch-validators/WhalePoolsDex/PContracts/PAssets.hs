module WhalePoolsDex.PContracts.PAssets (
    poolNftMintValidatorT,
    poolLqMintValidatorT,
) where

import WhalePoolsDex.PContracts.PApi (ownCurrencySymbol, tletUnwrap)
import PExtra.API (assetClass, assetClassValueOf, pValueLength)
...
poolNftMintValidatorT oref tn = plam $ \_ ctx -> unTermCont $ do
    ...
    let targetUtxoConsumed =
        ...
        correctTokensQty = (pValueLength # valueMint) #== 1
    pure $ targetUtxoConsumed #&& tokenMintExact #&& correctTokensQty
